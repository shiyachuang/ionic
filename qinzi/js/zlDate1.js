...
var pickerEvent = {
    Init: function (elemid) {
        ...
        document.getElementById("calendar_choose").style.top  = "105px";
...
var pickerHtml = {
    getHead: function () {
        var head = '<ul class="calendar_num basefix"><li class="bold">日</li><li>一</li><li>二</li><li>三</li><li>四</li><li>五</li><li class="bold">六</li></ul>';
        htmlObj.header = head;
    },
    getLeft: function () {
        var left = '<div class="calendar_left pkg_double_month"><p class="date_text"><b>零距离成长联盟</b><br/>' + obj.year + '年' + obj.month + '月</p><a href="javascript:void()" title="上一月" id="picker_last" class="pkg_circle_top">&lt;</a><a href="javascript:void()" title="下一月" id="picker_next" class="pkg_circle_bottom ">&gt;</a></div>';
        htmlObj.left = left;
    },
    getRight: function () {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                var state=commonUtil.getState((i - c));
                ...
                if (state != -1) {
                    classStyle = "class='on'";
                    priceStr = state ;
                    console.log(priceStr=="充足")
                    if (priceStr == "充足") {
                        classStyle = "class='on green'";
                        console.log(1)
                    }
                    else if (priceStr == "紧张") {
                        classStyle = "class='on yellow'";
                    } else  {
                        classStyle = "class='on red'";
                    }
                }
				...
				if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
					html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '"state="'+state+'"><a><span class="date basefix ">'+ (i -c) +'</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
				else{
                	html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price +'"state="'+state+ '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
...
var commonUtil = {
    ...
    getState: function (day) {
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].Date == dt) {
                return obj.priceArr[i].state.split('.')[0];
            }
        }
        return -1;
    },
    chooseClick: function (sender) {
        ...
        //     el.value = date;
			...
			alert("活动"+state);
            // pickerEvent.remove();
             window.location="choseNumber.html";
        // }
    }
}
/*本代码由素材家园原创，转载请保留网址：www.sucaijiayuan.com*/
